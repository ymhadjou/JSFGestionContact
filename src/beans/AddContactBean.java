	...
	public String checkInfosContact(){
		...
		//Contrôle que les champs sont non-vides
		 if (isMissing(firstName)||isMissing(lastName) || isMissing(email) || (id==0)) {
			 context.addMessage(null, new FacesMessage("champs ne peut être vide"));
			...
			if(context.getMessageList().size()>0)
				...
				cs.addContact(id, lastName, firstName, email); //ajout d'un contact avec les infos du formulaire
				return ("welcome-page");
			}
	}
